...
tf.flags.DEFINE_string("dataset", "Toy", "MIT or MNIST or Toy")
#tf.flags.DEFINE_string("data_dir", "Data_zoo/MIT_SceneParsing/", "path to dataset")
tf.flags.DEFINE_string("data_dir", "toy_data/", "path to dataset")
...
def folder_reader(data_dir):
    ...
    for sub in ['images/']:#, 'annotations/']:
        for sub_ in ['training/', 'validation/']:
            list = []
            for filename in glob.glob(data_dir + sub + sub_ + '*.png'):
                list.append({'annotation': filename.replace('images', 'annotations'),
...
def main(argv=None):
    ...
    if FLAGS.dataset == 'MIT':
        ...
        image_options = {'resize': True, 'resize_size': IMAGE_SIZE}
        train_records, valid_records = folder_reader(FLAGS.data_dir)
        train_dataset_reader = dataset.BatchDatset(train_records, image_options)
        image = tf.placeholder(tf.float32, shape=[FLAGS.batch_size, IMAGE_SIZE, IMAGE_SIZE, 3], name="input_image")
        #train_images, train_annotations = train_dataset_reader.next_batch(FLAGS.batch_size)
    ...
    tf.summary.image("ground_truth", tf.cast(annotation, tf.uint8)*255, max_outputs=2)
    tf.summary.image("pred_annotation", tf.cast(pred_annotation, tf.uint8)*255, max_outputs=2)
    ...
    if FLAGS.mode == "train":
        for itr in range(MAX_ITERATION):
            ...
            #train_annotations = train_annotations[:, :, :, 0]
            '''
            ...
            if itr % 50 == 0:
                ...
                #valid_annotations = valid_annotations[:, :, 0]
                '''
                for i in range(FLAGS.batch_size):
